...
namespace ES
{
    ...
    public class ESDataWindow : ESWindowBase_Abstract<ESDataWindow> //OdinMenuEditorWindow
    {
        ...
        public override void ES_LoadData()
        {
            ...
            if (EditorPrefs.HasKey("guidForCopyGroup"))
            {
                ...
                CopyGroup = ESDesignUtility.SafeEditor.LoadAssetByGUIDString<ScriptableObject>(guidForCopyGroup) ?? CopyGroup;
            }
        ...
        public override void ES_SaveData()
        {
            ...
            if (CopyGroup != null)
            {
                guidForCopyGroup = ESDesignUtility.SafeEditor.GetAssetGUID(CopyGroup) ?? guidForCopyGroup;
        ...
        private void Part_BuildSoPackPage(OdinMenuTree tree)
        {
            ...
            var allPacks = ESDesignUtility.SafeEditor.FindSOAssets<ISoDataPack>(TypeSelect);
        ...
        private void Part_BuildSoDataGroupPage(OdinMenuTree tree)
        {
            ...
            var allGroups = ESDesignUtility.SafeEditor.FindSOAssets<ISoDataGroup>(TypeSelect);
    ...
    public class Page_CreateNewSoPackOrSearch : ESWindowPageBase
    {
        ...
        {
            //找到全部数据组
            CachingAddGroups = ESDesignUtility.SafeEditor.FindSOAssets<ISoDataGroup>(ESDataWindowTool.GetGroupType(createPackType_));
        }
    ...
    public class Page_CreateNewGroupOrSearch : ESWindowPageBase
    {
        ...
        {
            //找到全部数据组
            var all = ESDesignUtility.SafeEditor.FindSOAssets<ISoDataGroup>(ESDataWindowTool.GetGroupType(createGroup_));
            foreach (var i in all)
            {
                if (i is ScriptableObject so)
                {
    ...
    public class Page_DataGroupOnChoose : ESWindowPageBase
    {
        ...
        public void CopyFromSoDataInfo()
        {
            ...
            {
                if(from!=null) ESDesignUtility.SafeEditor.DisplayDialog("类型不匹配！", $"想要拷贝单元数据为{from}({from?.GetType()})" +
                    $"，但是这个类型和当前数据组{group.getSoInfoType()}不符");
                else ESDesignUtility.SafeEditor.DisplayDialog("为空！", $"没有可用的拷贝源");
                return;
            }
        ...
        private void pastePersis()
        {
            if (ESDataWindow.CopyGroup != null)
            {
                ...
                if (groupcopy == null)
                {
                    ESDesignUtility.SafeEditor.DisplayDialog("为空！", $"没有可用的拷贝源<数据组不存在>");
                    return;
                }
                ...
                if (from == null || !group.getSoInfoType().IsAssignableFrom(from.GetType()))
                {
                    ...
                    else ESDesignUtility.SafeEditor.DisplayDialog("为空！", $"没有可用的拷贝信息<数据组>"+ groupcopy+"<键>"+ESDataWindow.CopyInfoKey);
                    return;
                }
                ScriptableObject copy = ScriptableObject.Instantiate(from);
                copy.name = DataFileName + DataKey + (hasChange ? "" : UnityEngine.Random.Range(0, 99999));
