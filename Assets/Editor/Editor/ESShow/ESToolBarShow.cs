        ...
        public static class CustomToolbarMenu
        {
            ...
            static void OnQuickSelectionToolbarGUI()
            {
                    ...
                    EditorStyles.toolbarDropDown, GUILayout.Width(300)))
                {
                    var menu = new GenericMenu();
                    var ss = EditorBuildSettings.scenes;
                    menu.AddItem(new GUIContent("快速选中<框架总文件夹>"), false, () =>
                    {
                    ...
                    menu.AddItem(new GUIContent("快速选中<So数据总文件夹>"), false, () =>
                    {
                    ...
                    menu.AddSeparator("");
                    menu.AddItem(new GUIContent("快速选中<玩家对象>"), false, () =>
                    {
                        var player = GameObject.FindGameObjectWithTag("Player");
                        if (player != null) Selection.activeGameObject = player;
                    });
                    menu.AddItem(new GUIContent("快速选中<纯编辑器管理器>"), false, () =>
                    {
                        var it = UnityEngine.Object.FindAnyObjectByType<ESEditorOnlyPartMaster>();
                        if (it != null) Selection.activeGameObject = it.gameObject;
                    });
                    menu.AddItem(new GUIContent("快速选中<游戏核心管理器>"), false, () =>
                    {
                        var it = UnityEngine.Object.FindAnyObjectByType<GameCenterManager>();
                        if (it != null) Selection.activeGameObject = it.gameObject;
                    });
                    menu.AddItem(new GUIContent("快速选中<对象池管理器>"), false, () =>
                    {
                        var it = UnityEngine.Object.FindAnyObjectByType<ES_PoolMaster>();
                        if (it != null) Selection.activeGameObject = it.gameObject;
                    });
                    menu.AddItem(new GUIContent("快速选中<不死联网核心管理器>"), false, () =>
                    {
