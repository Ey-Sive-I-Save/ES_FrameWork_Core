...
namespace ES.Pointer
{
    ...
    public class PFBouns_BuffKeyGetter : PointerForBonus
    {
       ...
       /* [LabelText("使用的Buff键")]
        ...
        public override object Pick(object on= null, object from = null, object with = null)
        {
            ...
            if (player != null)
            {
                player.BuffDomain?.buffHosting.AddHandle(ESStaticUtility.Creator.CreateBuffRunTimeByKey(buffKey, startStatus?.Pick()));
            }
            return base.Pick(On, From, With);
        }*/
    }
    ...
    public class PointerPickerFeelMMFPlayer_AllCOntrol : PointerOnlyAction
    {
        ...
        [ShowIfGroup("显示float", VisibleIf = "@ShowFloat()"), SerializeReference, LabelText("", Text = "@FloatName()"), GUIColor("@ESStaticUtility.ColorSelector.Color_03")]
        public IPointerForFloat_Only valueFloat_ = new PointerForFloat_Direct() { float_ = 1 };
        [ShowIfGroup("显示v3", VisibleIf = "@option==EnumCollect.PointerMMFPlayerHandleOptions.SetRangeCenterAndDistance"), SerializeReference, LabelText("", Text = "生效的范围中心引用"), GUIColor("@ESStaticUtility.ColorSelector.Color_04")]
