...
namespace ES
{
    ...
    public class ArchPool : IArchitecture, IInittable
    {
        ...
        public void SetBool(string key, bool use, EnumCollect.HandleTwoBoolFunction function = EnumCollect.HandleTwoBoolFunction.Set)
        {
            if (_ArchValues.TryGetValue(key, out var vv))
            {
                vv.SetBool(ESStaticDesignUtility.Function.FunctionForHandleTwoBool(vv.GetBool(), use, function));
            }
            else
            {
               AddArchValueTruelyToCreate(key, new ArchitectureTypeValue_Bool() { Value = ESStaticDesignUtility.Function.FunctionForHandleTwoBool(true, use, function) });
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void SetFloat(string key, float use, EnumCollect.HandleTwoNumberFunction function = EnumCollect.HandleTwoNumberFunction.Set)
        {
            if (_ArchValues.TryGetValue(key, out var vv))
            {
                vv.SetFloat(ESStaticDesignUtility.Function.FunctionForHandleTwoFloat(vv.GetFloat(), use, function));
            }
            else
            {
                AddArchValueTruelyToCreate(key, new ArchitectureTypeValue_Float() { Value = ESStaticDesignUtility.Function.FunctionForHandleTwoFloat(0, use, function) });
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void SetInt(string key, int use, EnumCollect.HandleTwoNumberFunction function = EnumCollect.HandleTwoNumberFunction.Set)
        {
            if (_ArchValues.TryGetValue(key, out var vv))
            {
                vv.SetInt(ESStaticDesignUtility.Function.FunctionForHandleTwoInt(vv.GetInt(), use, function));
            }
            else
            {
                AddArchValueTruelyToCreate(key, new ArchitectureTypeValue_Int() { Value = ESStaticDesignUtility.Function.FunctionForHandleTwoInt(0, use, function) });
            }
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
