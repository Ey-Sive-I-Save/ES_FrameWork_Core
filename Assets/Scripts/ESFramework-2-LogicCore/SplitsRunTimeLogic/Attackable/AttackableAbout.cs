...
namespace ES
{
    ...
    public class Attackable
    {
        ...
        [FoldoutGroup("条件和类型"), LabelText("攻击类型标识")] public EnumCollect.EntityAttackBaseType attackType = EnumCollect.EntityAttackBaseType.Melle;
        [FoldoutGroup("条件和类型"), LabelText("攻击对目标的要求")] public EnumCollect.EntityTryStartAttackCondition require = EnumCollect.EntityTryStartAttackCondition.MustTargetInRange;
        [FoldoutGroup("条件和类型"), LabelText("攻击触发机制")] public EnumCollect.EntityAttackOnHappenCondition attackOn = EnumCollect.EntityAttackOnHappenCondition.Direct;
        ...
        public void TryAttackOn(Entity Who, Entity Target = null, Item with = null)
        {
            ...
            else if (require == EnumCollect.EntityTryStartAttackCondition.MustTargetAlive)
            {
                if (target == null) return;
            }
            //Require通过√
            if (attackType.HasFlag(EnumCollect.EntityAttackBaseType.Melle))
            {
