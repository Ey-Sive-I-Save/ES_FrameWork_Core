            ...
            public static void EasyHandleField<T>(object o, string field, T t, EnumCollect.HandleTwoNumberFunction func, Type type = null)
            {
                ...
                if (fieldInfo != null)
                {
                    ...
                    if (t is float f && fieldInfo.FieldType.IsAssignableFrom(typeof(float)))
                    {
                        ...
                        float right = f;
                        fieldInfo.SetValue(o, ESStaticDesignUtility.Function.FunctionForHandleTwoFloat(left, right, func));
                    }
                    else if (t is int i && fieldInfo.FieldType.IsAssignableFrom(typeof(int)))
                    {
                        int left = (int)Convert.ToSingle(fieldInfo.GetValue(o));
                        int right = i;
                        fieldInfo.SetValue(o, (int)ESStaticDesignUtility.Function.FunctionForHandleTwoFloat(left, right, func));
                    }
                    ...
                    else if (t is int lay && typeof(LayerMask).IsAssignableFrom(fieldInfo.FieldType))
                    {
                        ...
                        int right = lay;
                        fieldInfo.SetValue(o, (LayerMask)ESStaticDesignUtility.Function.FunctionForHandleTwoFloat(left, right, func));
                    }
            ...
            public static void EasyHandleProperty<T>(object o, string field, T t, EnumCollect.HandleTwoNumberFunction func)
            {
                ...
                if (propertyInfo != null)
                {
                    ...
                    if (t is float f && propertyInfo.PropertyType.IsAssignableFrom(typeof(float)))
                    {
                        ...
                        float right = f;
                        propertyInfo.SetValue(o, ESStaticDesignUtility.Function.FunctionForHandleTwoFloat(left, right, func));
                    }
                    //开始处理 
                    else if (t is int i && propertyInfo.PropertyType.IsAssignableFrom(typeof(int)))
                    {
                        ...
                        int right = i;
                        propertyInfo.SetValue(o, (int)ESStaticDesignUtility.Function.FunctionForHandleTwoFloat(left, right, func));
                    }
