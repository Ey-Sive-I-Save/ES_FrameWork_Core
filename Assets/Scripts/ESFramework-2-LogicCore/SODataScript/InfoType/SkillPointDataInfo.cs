...
namespace ES
{
    ...
    public class SkillPointDataInfo : SoDataInfo
    {
        ...
        {
            [Space(25)]
            [LabelText("显示名称",SdfIconType.Alarm),GUIColor("@ESStaticDesignUtility.ColorSelector.Color_03"), PropertyOrder(-1)] public string displayName = "技能点-";
            
           /* [DisplayAsString(fontSize: 22),ShowInInspector,HideLabel,PropertyOrder(-1)] public string s_ => "↑↑↑应该被作为动态标签判据↑↑↑";
            //[LabelText("显示图标集合")]
            [InlineProperty(LabelWidth =50),Space(10),TabGroup("<1>无->未知",TextColor = "@ESStaticDesignUtility.ColorSelector.Color_03")]
            ...
            [InlineProperty(LabelWidth = 50), Space(10), TabGroup("<2>未知->不允许解锁", TextColor = "@ESStaticDesignUtility.ColorSelector.Color_01")]
            ...
            [InlineProperty(LabelWidth = 50), Space(10), TabGroup("<3>不允许解锁->条件未达到", TextColor = "@ESStaticDesignUtility.ColorSelector.Color_03")]
            ...
            [InlineProperty(LabelWidth = 50), Space(10), TabGroup("<4>条件未达到->条件达成", TextColor = "@ESStaticDesignUtility.ColorSelector.Color_03")]
            ...
            [InlineProperty(LabelWidth = 50), Space(10), TabGroup("<5>条件达成->解锁", TextColor = "@ESStaticDesignUtility.ColorSelector.Color_01")]
            [LabelText("从 条件完全达成 到 解锁", SdfIconType.Clipboard)] public ADefaultBoolAndBoolAKVP forCanUnlockComplete2Unlock = new ADefaultBoolAndBoolAKVP() { defaultForNull = false };
            [InlineProperty(LabelWidth = 50), Space(10), TabGroup("<6>解锁操作", TextColor = "@ESStaticDesignUtility.ColorSelector.Color_03")]
            [LabelText("解锁时触发的操作", SdfIconType.UnlockFill)] public PointerNonePackOnlyActionByArKVP_LoopOnce whenUnlock = new PointerNonePackOnlyActionByArKVP_LoopOnce() { pointers=new List<PointerNoneByArchitectureKeyValuePoolTypeListIOC>() { new PointerNoneByArKVP_FloatInPool() { function= EnumCollect.HandleTwoNumberFunction.Sub },new PointerNoneByArKVP_AddTagInPool() {  } } };
            [InlineProperty(LabelWidth = 50), Space(10), TabGroup("<6>解锁操作", TextColor = "@ESStaticDesignUtility.ColorSelector.Color_03")]
            [LabelText("取消时触发的操作", SdfIconType.LockFill)] public PointerNonePackOnlyActionByArKVP_LoopOnce whenCancel_ = new PointerNonePackOnlyActionByArKVP_LoopOnce() { pointers = new List<PointerNoneByArchitectureKeyValuePoolTypeListIOC>() { new PointerNoneByArKVP_FloatInPool() { function = EnumCollect.HandleTwoNumberFunction.Add }, new PointerNoneByArKVP_RemoveTagInPool() { } } };

            [InlineProperty(LabelWidth = 50), Space(10), TabGroup("<7>应用操作", TextColor = "@ESStaticDesignUtility.ColorSelector.Color_03")]
            [LabelText("应用时触发的操作", SdfIconType.UnlockFill)] public PointerNonePackOnlyActionByArKVP_LoopOnce whenApply_ = new PointerNonePackOnlyActionByArKVP_LoopOnce() { pointers = new List<PointerNoneByArchitectureKeyValuePoolTypeListIOC>() { new PointerNoneByArKVP_IntInPool() {function= EnumCollect.HandleTwoNumberFunction.Set } } };
            [InlineProperty(LabelWidth = 50), Space(10), TabGroup("<7>应用操作", TextColor = "@ESStaticDesignUtility.ColorSelector.Color_03")]
            [LabelText("取消应用时触发的操作", SdfIconType.LockFill)] public PointerNonePackOnlyActionByArKVP_LoopOnce whenDeApply_ = new PointerNonePackOnlyActionByArKVP_LoopOnce() { pointers = new List<PointerNoneByArchitectureKeyValuePoolTypeListIOC>() { new PointerNoneByArKVP_IntInPool() { function = EnumCollect.HandleTwoNumberFunction.Set } } };

*/


            [InlineProperty(LabelWidth = 50), LabelText("状态图标"), TabGroup("<->精灵图配置", TextColor = "@ESStaticDesignUtility.ColorSelector.Color_04"), OdinSerialize,NonSerialized]public SkillPointSprites sprites = new SkillPointSprites();
